// MAAHI_GROUPS are the user-facing buckets a future picker UI will surface.
// The 16 underlying capabilities still exist and still route in the hidden
// capability router — the groups are organizational metadata, not a behavior
// change. When/if we ship a picker, we render groups and reveal capabilities
// inside on expand.
export const MAAHI_GROUPS = [
  { id: "listen",         label: "Listen",            description: "Hold the feeling first." },
  { id: "patterns",       label: "See your patterns", description: "Name the loop without judgment." },
  { id: "reply",          label: "Reply",             description: "Say it without shrinking." },
  { id: "hold_the_line",  label: "Hold the line",     description: "Boundaries and self-respect, intact." },
  { id: "decide",         label: "Decide",            description: "Make the next move obvious." },
  { id: "plan_celebrate", label: "Plan & celebrate",  description: "From spark to debrief to win." },
  { id: "culture",        label: "Family & Culture",  description: "Handle the big-life conversations." },
] as const;

export type MaahiGroup = (typeof MAAHI_GROUPS)[number]["id"];
...
export function getCapabilitiesByGroup(group: MaahiGroup): MaahiCapabilityDetails[] {
  return MAAHI_CAPABILITIES.filter((c) => c.group === group);
}
